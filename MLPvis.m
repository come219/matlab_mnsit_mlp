...
m = m.initializeWeightsRandomly(initRange);
    
% initialize hidden neuron visualization
for i=1:hiddenSize
    points = perceptronBoundary(m.hiddenLayerWeights(i,1:3), 0);
    strength = m.outputLayerWeights(1,i);
...
for t = 1:10000
    for i=1:speedUp
        ...
        m.train_single_data(X(:,index), Y(index), 0.05);
    end
